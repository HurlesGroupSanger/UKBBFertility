...
First step to analyse UK Biobank WES data is to actually acquire the variant files themselves. Please see [this page](http://biobank.ndph.ox.ac.uk/showcase/field.cgi?id=23160) for additional information on accquiring required files. Please note that conversion to VCF format requires [PLINKv1.9 or PLINKv2](https://www.cog-genomics.org/plink2) and [tabix](http://www.htslib.org/doc/tabix.html). Roughly, files were accquired and processed into a VCF format file with the following commands:
...
## Note that you must have your keyfile provided by UKB in the same directory/pointed to with the -a flag
...
plink --bed ukb_efe_chr1_v1.bed --bim ukb_fe_exm_chrall_v1.bim --fam ukb44165_efe_chr1_v1_s49959.fam --out ukbb_plink.vcf --recode vcf
...
* gnomAD allele frequencies [gnomad] -- Note that I am using AF_nfe, which is non-Finnish European AF
* MPC scores [mpc]
