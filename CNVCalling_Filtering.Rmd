...
load.package("biomaRt") ## For getting gene lists/data
load.package("readxl") ## For reach male infertility genes
...
calculate.cutoff <- function(cutoff, data) {
    ...
    print(ggplot() + geom_text(data=calcd.cutoffs,aes(spec,sens,colour=ct,label=metric),size=6) +
            geom_text(data=calcd.cutoffs.os,aes(spec.mean,sens.mean,colour=ct,label=training.type),size=6) +      
            scale_colour_manual(values=c(del.line,dup.line)) + 
            scale_x_reverse(lim=c(1,0),name="Specificity") + 
            scale_y_continuous(lim=c(0,1),name="Sensitivity") + theme)
...
## 5F. Print and Save Final QCd UKBB CNVs 
...
3. Generate a tab delimited file that can be used when running `PhenotypeTesting.Rmd` (`scripts/attach_gene_stats.pl`). This calculates:
    ...
    + s~het~ scores without disease genes, male infertility genes, or mouse infertility genes
...
echo -e "rawdata/genelists/diseaseGenes.txt\nrawdata/genelists/male_infertility_genes.txt\nrawdata/genelists/mouse_infertility_genes.annotated.txt" > rawdata/genelists/gene_lists.txt

# This just makes a one-liner runs all the files at once, can do it however you like
perl -e 'for (1..457) {my $num = sprintf("%03d", $_); print "./attach_gene_stats.pl vep_input/vep_annotation.$num.txt > vep_input_revision/vep_parsed.$num.txt;\n";}' | bash

## Cat it all together!
cat vep_input_revision/vep_parsed.*.txt | sort -k 1,1 -k2,2n > cnv_vep_parsed.revision.sorted.bed
```

## 7C. Build Final Files for Phenotype Testing
