...
library(dplyr) ## for mutating data.tables
library(tidyr) ## for mutating data.tables
library(broom) ## Makes getting covars out of lm much tidier
library(stringr) ## Random string manipulation functions
...
## 3H. Basic Stats About UK Biobank Participants

Goal of this section is to print any basic demographic stat available just from phenotype data:
...
#/nfs/users/nfs_e/eg15/scripts/matcher.pl -file1 ../hg19.genes.txt -file2 male_infertility_genes.txt -col1 5 -r | sed 's_^M__g' | perl -ane 'chomp $_; splice(@F, 5, 10); print join("\t", @F) . "\n";' > male_infertility_genes.annotated.txt
...
expression <- fread("/Users/eg15/Documents/Completed Projects/DDD-RT/Manuscript/Supplement/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct")
...
build.omics.map <- function(cnv.file, id.name, merge.key) {
  ...
  annotated.cnvs.qcd[,gt:=ifelse(Copy_Number == 0 | Copy_Number == 4,2,1)]
...
for (t in c("DEL","LOF_HC")) {
  ...
  print(paste0(t, " ≥ 0.15 sHET Prop Male (", sprintf("%0.2f",prop[1,2]),"), Female (", sprintf("%0.2f",prop[2,2]),"), p = ", sprintf("%0.2f", fisher$p.value)))
  
}
```

# 5. Variant Impact on Traits
...
$ trait = N_{variants} + age + age^2 + PC1..PC10 $

Where

$ N_{variants}$

is number of qualifying (e.g. DEL, DUP, LoF, MIS, SYN) variants per individual. This block will also switch from linear to logistic based on a flag given.
...
run.regression <- function(maf,
  ...
  if (cutoff.high == T & (variant.type == "DEL" | variant.type == "DUP")) {
...
#### Excluding Male Infertility Genes
...
#### Excluding Pathogenic CNV Carriers
...
test <- ukbb.annotated.cnvs.qcd[eid %in% samples.UKBB.cnv[,eid] & filter.0.95.wes.support.score == T & path.locus != "null"]

paste0("CNV Carriers account for ",sprintf("%0.1f",(length(unique(test[,eid]))/nrow(samples.UKBB.cnv)*100)),"% (", length(unique(test[,eid])),") of individuals.)")
...
plot.a <- make.meta.table(results.fertility, F, gene.list = "product_sHET", b = 0.25)
...
linear.data[[1]][Sex == "Male" & variant.type == "META",paste0("Males have ",sprintf("%0.2f",abs(var.beta))," fewer children", " [95% CI ",sprintf("%0.2f",abs(var.beta+(1.96*var.stderr))), "-",sprintf("%0.2f",abs(var.beta-(1.96*var.stderr))),"]", " p=",if_else(var.p<=1e-2,sprintf("%0.1e",var.p),sprintf("%0.2f",var.p)))]

linear.data[[1]][Sex == "Female" & variant.type == "META",paste0("Females have ",sprintf("%0.2f",abs(var.beta))," fewer children", " [95% CI ",sprintf("%0.2f",abs(var.beta+(1.96*var.stderr))), "-",sprintf("%0.2f",abs(var.beta-(1.96*var.stderr))),"]", " p=",if_else(var.p<=1e-2,sprintf("%0.1e",var.p),sprintf("%0.2f",var.p)))]
