...
fertility.metrics <- UKBB.raw.phenotypes[,c("eid",
                                            ...
                                            "2149-0.0", ## Number of Sexual Partners
...
fertility.metrics[,num.partners:=if_else(is.na(had.sex), -9,
                                         if_else(had.sex == 0, 0,
                                                 if_else(num.partners > 0 & num.partners <= 100,num.partners,-9)))]


ggplot(fertility.metrics,aes(num.partners,group=sexPulse,fill=sexPulse)) +
  stat_count(position="identity",alpha=0.5) +
  scale_alpha_continuous(range=c(0,1)) +
  scale_x_continuous(name="Number of Sexual Partners", breaks = c(-9, 0, 25, 50, 75, 100), labels = c("Did not answer", 0, 25, 50, 75, 100)) +
  sex.colours.fill +
  theme.legend

## Now convert all the -9s back to NA
fertility.metrics <- fertility.metrics[,c("eid","children.fathered","live.births","partner.in.house","same.sex","had.sex","num.partners")]
...
fertility.metrics[,num.partners:=if_else(num.partners==-9,as.numeric(NA),num.partners)]
...
curl -o rawdata/ukbb_missing_baits/xgen_plus_spikein_b38_alt_affected.bed https://biobank.ctsu.ox.ac.uk/crystal/crystal/auxdata/xgen_plus_spikein_b38_alt_affected.bed
...
run.regression <- function(maf,
  ...
  covariates <- c(gene.list,add.covars,"agePulse.squared","agePulse","sexPulse")
  if (num.pcs == 30) {
  ...
  ## NAGLEKERKE STUFF
  if (nagel == T) {
    covariates <- c("agePulse.squared","agePulse","sexPulse",add.covars,"PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10","PC11","PC12","PC13","PC14","PC15","PC16","PC17","PC18","PC19","PC20","PC21","PC22","PC23","PC24","PC25","PC26","PC27","PC28","PC29","PC30")
  
    cov.string <- paste(covariates, collapse=" + ")
    formated.formula <- as.formula(paste(y.var, cov.string,sep=" ~ "))
  
    test.lm.init <- glm(formated.formula, data=final.stats, family=fam)
...
results.fertility.no.male.infertility[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, indv.to.exclude = ukbb.has.male.infertility),by=1:nrow(results.fertility.no.male.infertility)]
...
results.fertility.no.path[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, indv.to.exclude = ukbb.path.carriers),by=1:nrow(results.fertility.no.path)]
...
for (i in c(40,50,60)) {
  ...
  results.fertility.age[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T, indv.to.exclude = age.remove),by=1:nrow(results.fertility.age)]
...
## Calculate the 'MHT' binary listed above:
UKBB.phenotype.data[,mht.binary:=if_else(eid %in% has.mht, 1, 0)]

results.mht <- data.table(crossing(maf = c(0.001,0),
                                  gene.list = "product_sHET",
                                  y.var = c("fi.developmental_disorder","fi.asd","fi.add","fi.scizo","fi.bipolar","hes.developmental_disorder","hes.asd","hes.add","hes.scizo","hes.bipolar","mht.binary"),
                                  sex = c(1,2),
                                  variant.type = c("DEL","LOF_HC")))

results.mht[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T),by=1:nrow(results.mht)]

plot.result(results.mht[y.var == "mht.binary"],T,10,0,75,"# of Individuals\nWith sHET\n> 0.15")

## This does the same tests but ONLY for individuals with GP data (using the `indv.to.exclude` flag)
results.mht.gp.only <- data.table(crossing(maf = c(0),
                                  gene.list = "product_sHET",
                                  y.var = c("fi.developmental_disorder","fi.asd","fi.add","fi.scizo","fi.bipolar"),
                                  sex = c(1,2),
                                  variant.type = c("DEL","LOF_HC")))

results.mht.gp.only[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,indv.to.exclude = UKBB.phenotype.data[has.gp.data == 0, eid]),by=1:nrow(results.mht.gp.only)]
```
...
results.fertility.no.mhq[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,indv.to.exclude = has.mht),by=1:nrow(results.fertility.no.mhq)]
...
results.has.CHOD <- data.table(crossing(maf = c(0.001,0),
                                      gene.list = "product_sHET",
                                      y.var = c("has.gp.data"),
                                      variant.type = c("DEL","DUP","LOF_HC","SYN","MIS"),
                                      sex = c(1,2)))

results.has.CHOD[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T),by=1:nrow(results.has.CHOD)]
```
...
results.fertility.no.same.sex[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, indv.to.exclude = ubb.same.sex),by=1:nrow(results.fertility.no.same.sex)]

plot.result(results.fertility.no.same.sex,T,20,0,1.4,"# of Individuals\nWith sHET\n> 0.15")
```

## 5J. Sexual Behaviour Phenotypes
...
## Had sex at all
results.had.sex <- data.table(crossing(maf = c(0.001,0),
                                   gene.list = c("product_sHET"),
                                   y.var = "had.sex",
                                   variant.type = c("DEL","DUP","LOF_HC","SYN","MIS"),
                                   sex = c(1,2)))

results.had.sex[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = F),by=1:nrow(results.had.sex)]

plot.result(results.had.sex,T,20,0,1.4, "# of Individuals\nWith sHET\n> 0.15")

## Number of partners
results.num.partners <- data.table(crossing(maf = c(0.001, 0),
                                            gene.list = c("product_sHET"),
                                            y.var = "num.partners",
                                            variant.type = c("DEL","DUP","LOF_HC","SYN","MIS"),
                                            sex = c(1,2)))

results.num.partners[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,F,return.data = F),by=1:nrow(results.num.partners)]
```

## 5K. Neutral Phenotypes
...
## HES Models:
bsub -q normal -M 2500 -o gridout/logistic.%J.%I -J 'LOG[1-19154]%500' './run_logistic_model.R rawdata/phewas/samples.hes.rdat 0 100 FALSE outfiles_hes/'
...
```{r MIC, fig.height=3, fig.width=8.5}

## Build columns in the phenotype data that represent presence of infertility codes:
...
UKBB.phenotype.data[,hes.fert:=if_else(sexPulse == 1,
                                     if_else(eid %in% hes.data.long[grepl("N46",icd.category),eid], 1, 0),
                                     if_else(eid %in% hes.data.long[grepl("N97",icd.category),eid], 1, 0))]

## CHOD Analyses
ukbb.noCHOD.individuals <- UKBB.phenotype.data[has.gp.data==0, eid]

results.fertility.MIC.CHOD <- data.table(crossing(maf = c(0.001,0),
                                                gene.list = "product_sHET",
                                                y.var = c("fi.fert"),
                                                variant.type = c("DEL","DUP","LOF_HC","SYN","MIS"),
                                                sex = c(1,2)))

results.fertility.MIC.CHOD[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T),by=1:nrow(results.fertility.MIC.CHOD)]

## And do HES...
results.fertility.MIC.hes <- data.table(crossing(maf = c(0.001,0),
                                                gene.list = "product_sHET",
                                                y.var = c("hes.fert"),
                                                variant.type = c("DEL","DUP","LOF_HC","SYN","MIS"),
                                                sex = c(1,2)))

results.fertility.MIC.hes[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T),by=1:nrow(results.fertility.MIC.hes)]
```
...
#### Fertility for GP Only Individuals

Check to see if just using individuals with GP data (~50% of all UKB Participants) changes the effect size of our primary model of $ has.children \sim s_{het[i,v]} $.

```{r CHOD Only, fig.height=8, fig.width=7}

ukbb.noCHOD.individuals <- UKBB.phenotype.data[has.gp.data==0, eid]

results.fertility.CHOD.only <- data.table(crossing(maf = c(0.001,0),
                                                 gene.list = "product_sHET",
                                                 y.var = c("children.fathered","live.births"),
                                                 variant.type = c("DEL","DUP","LOF_HC","SYN","MIS")))
results.fertility.CHOD.only[,sex:=if_else(y.var=="children.fathered",1,2)]

results.fertility.CHOD.only[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, indv.to.exclude = ukbb.noCHOD.individuals),by=1:nrow(results.fertility.CHOD.only)]

plot.result(results.fertility.CHOD.only,T,20,0,1.4,"# of Individuals\nWith sHET\n> 0.15")

## Also do the same for the MHQ to demonstrate difference...
ukbb.noMHQ.individuals <- UKBB.phenotype.data[mhq.answered_mhq==0, eid]

results.fertility.MHQ.only <- data.table(crossing(maf = c(0.001,0),
                                                 gene.list = "product_sHET",
                                                 y.var = c("children.fathered","live.births"),
                                                 variant.type = c("DEL","DUP","LOF_HC","SYN","MIS")))
results.fertility.MHQ.only[,sex:=if_else(y.var=="children.fathered",1,2)]

results.fertility.MHQ.only[,c("var.beta","var.stderr","var.p","n.var","n.indvs"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, indv.to.exclude = ukbb.noMHQ.individuals),by=1:nrow(results.fertility.MHQ.only)]

plot.result(results.fertility.MHQ.only,T,20,0,1.4,"# of Individuals\nWith sHET\n> 0.15")
```
...
calc.base <- function(s, with.zero, childlessness) {
  ...
  final.data <- rbind(final.data,merge(tab[has.wes == F],variant.counts[type=="DEL" & allele.freq==0,c("sample_id","product_sHET","type")],by.x="eid",by.y="sample_id"))
...
## Do the null model
final.results.matrix <- data.table(crossing(maf = c(0),
                                   gene.list = c("product_sHET"),
                                   y.var = c("children.fathered","live.births"),
                                   variant.type = c("DEL", "LOF_HC"),
                                   curr.cov = list("")))
final.results.matrix[,sex:=if_else(y.var=="children.fathered",1,2)]

final.results.matrix[,c("var.beta","var.stderr","var.p","n.var","n.indvs","model","inc.r.shet"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, nagel = T),by=1:nrow(final.results.matrix)]

for (s in c(1,2)) {
  
  meta.table <- final.results.matrix[(variant.type == "DEL" | variant.type == "LOF_HC") & sex == s]
  meta.analy <- metagen(var.beta,
                        var.stderr,
                        studlab = variant.type,
                        method.tau = "SJ",
  ...
  meta.table <- data.table(maf = 0,
                           gene.list = "product_sHET",
                           y.var = if_else(s == 1, "children.fathered","live.births"),
                           variant.type = "META",
                           curr.cov = list(""),
                           sex = s,
                           ...
                           n.var = NaN,
                           n.indvs = final.results.matrix[(variant.type == "DEL" | variant.type == "LOF_HC") & sex == s,sum(n.indvs)],
                           model = list(),
                           inc.r.shet = NaN)
  final.results.matrix <- rbind(final.results.matrix, meta.table)
...
for (i in c(1:length(add.traits.all))) {
  ...
  results.matrix <- data.table(crossing(maf = c(0),
                                   gene.list = c("product_sHET"),
                                   y.var = c("children.fathered","live.births"),
                                   variant.type = c("DEL", "LOF_HC"),
                                   curr.cov = list(curr.cov)))
  results.matrix[,sex:=if_else(y.var=="children.fathered",1,2)]

  results.matrix[,c("var.beta","var.stderr","var.p","n.var","n.indvs","model","inc.r.shet"):=run.regression(maf,gene.list,y.var,sex,variant.type,T,return.data = T, add.covars = curr.cov, nagel = T),by=1:nrow(results.matrix)]
  
  for (s in c(1,2)) {
    meta.table <- results.matrix[(variant.type == "DEL" | variant.type == "LOF_HC") & sex == s]
    meta.analy <- metagen(var.beta,
                          var.stderr,
                          studlab = variant.type,
                          method.tau = "SJ",
    ...
    meta.table <- data.table(maf = 0,
                             ...
                             var.p = meta.analy$pval.fixed,
                             n.var = NaN,
                             n.indvs = results.matrix[(variant.type == "DEL" | variant.type == "LOF_HC") & sex == s,sum(n.indvs)],
                             model = list(),
                             inc.r.shet = NaN)
    results.matrix <- rbind(results.matrix, meta.table)
  }
  
  final.results.matrix <- rbind(final.results.matrix, results.matrix)
...
make.meta.table <- function(data, is.linear, 
  ...
  plot <- ggplot(meta.table,aes(variant.type,var.beta,group=Sex,colour=Sex)) +
    ...
    geom_text(aes(y = p.pos, label=paste0("p==",if_else(var.p<1e-2,paste0(prefix,"%*%10^",exponent),sprintf("%0.2f",var.p)))), parse = T,position=position_dodge(0.7),size=4,hjust=1,show.legend = F) +
...
```{r Main Text Figure 1a}

## This just gets a forest plot for our primary childlessness logistic regression
plot.a <- make.meta.table(results.fertility, F, b = 0.25,p.pos=0.1)
...
```{r Main Text Figure 1, fig.height=9, fig.width=8}
...
```{r Main Text Figure 2, fig.height=7, fig.width=10}
...
plot.medical <- function(s, l, show.x = F, chod.title = "", hes.title = "") {
  ...
  if (s == "MALE") {
    lims = c(12.5,13.2)
    brs = c(12.6,12.8,13.0,13.2)
    p.thresh = 12.85
  } else {
    lims = c(1.25,1.5)
    brs = c(1.26,1.34,1.42,1.50)
    p.thresh = 1.37
  }
  if (show.x == T) {
...
male.plots <- plot.medical("MALE",3,"Complete Health Outcomes Data","Hospital Episode Stats.")
...
main.plot <- male.plots[[1]] + male.plots[[2]] + female.plots[[1]] + female.plots[[2]] + plot_layout(ncol = 2, nrow = 2, widths = c(16, 6), tag_level = 'new') + plot_annotation(tag_levels = "A")

figure.2 <- wrap_elements(y.lab) + main.plot + plot_layout(ncol = 2, nrow = 1, widths = c(0.3,20), tag_level = 'keep')
figure.2

ggsave("figures/Figure2.svg",figure.2,width=10,height=7)
...
```{r Main Text Figure 3, fig.height=12, fig.width=7}

fig.partner <- make.meta.table(results.partner, F, title = "Partner at Home",show.x=F,b=0.25,ymin=-0.3,p.pos=-.05,block=-.25)
fig.had.sex <- make.meta.table(results.had.sex, F, title = "Ever Had Sex", show.x=F,b=0.25,ymin=-0.3,p.pos=-.05,block=-.25)
fig.ea <- make.meta.table(results.ea, F, title = "University Degree",b=0.25,ymin=-0.3,p.pos=-.05,block=-.25)
fig.hhi <- make.meta.table(results.household.income, T, title = "Household Income",show.x=F, ymin = -2.6, ymax= 0.5, block = -2.5, p.pos = -2.1, b = 0.5)
fig.cog <- make.meta.table(results.cog[sex==1 | sex == 2], T, title = "Fluid Intelligence", ymin = -2.6, ymax= 0.5, block = -2.5, p.pos = -2.1, b = 0.5)
fig.mht <- make.meta.table(results.mht[y.var == "mht.binary"], F, title = "Mental Health Disorder", b = 25, ymin = -0.3, ymax = 75, p.pos = 0.04,alt.y.axis = expression(bold(log(Odds~Ratio)~at~s[het]~burden==1)))

fig.mht[[2]] <- fig.mht[[2]] +
  scale_y_log10(limits = c(0.01,75),breaks = c(0.1,1,10,70), name = expression(bold(Odds~Ratio~at~s[het]~burden==1)))

# labels = c("0.2","1","10","50","100")

## And mash it all together...
figure.3 <- fig.partner[[2]] + fig.had.sex[[2]] + fig.ea[[2]] + fig.mht[[2]] + fig.hhi[[2]] + fig.cog[[2]]  + plot_layout(nrow=6, guides = "collect") + plot_annotation(tag_levels = "A")
figure.3

ggsave("figures/Figure3.svg",figure.3,width=7,height=12)
...
fitness.redux <- ggplot(model.fertility, aes(x = shet, group = sexPulse, fill = sexPulse)) + 
  ...
  theme.figures.legend + theme(axis.title = element_text(size = 12), axis.text = element_text(size = 12))
...
```{r Multiple Regression, fig.height=5.5, fig.width=9}

final.results.matrix[,curr.cov.string:=factor(curr.cov.string, levels = final.results.matrix[,unique(curr.cov.string)])]

figure.4.subset.plot <- final.results.matrix[grepl("Fluid Intelligence", curr.cov.string) == F & (n.terms <= 1 | n.terms == 5 | curr.cov.string == "Has Partner, Ever Had Sex"), ]
...
ors.plot <- ggplot(figure.4.subset.plot[variant.type == "META"], aes(curr.cov.string, or, group = sexPulse, colour = sexPulse)) +
  ...
  geom_errorbar(aes(ymin = var.ci.lower, ymax = if_else(var.ci.upper > 1.25, 1.25, var.ci.upper)), width = 0, position = position_dodge(0.5)) +
  geom_text(aes(y = -0.13, label = paste0("p==",if_else(var.p<1e-2,paste0(prefix,"%*%10^",exponent),sprintf("%0.2f",var.p)))), parse = T, position = position_dodge(0.8), hjust = 0,size = 4) +
  scale_x_discrete(name = "") +
  ...
  scale_size_area(breaks=c(150000,175000,180000),guide=guide_legend(title="# of Indivs."), limits = c(0, 200000),) +
  sex.colours.colour +
  coord_flip() +
...
r2.plot.table <- figure.4.subset.plot[variant.type == "DEL"]
r2.plot.table[,val:=if_else(sexPulse == "Male",
                            inc.r.shet/r2.plot.table[sex == 1 & curr.cov == "",inc.r.shet],
                            inc.r.shet/r2.plot.table[sex == 2 & curr.cov == "",inc.r.shet])]
...
inc.r.shet.plot <- ggplot(r2.plot.table,aes(curr.cov.string, val, group = sexPulse, fill = sexPulse)) +
  ...
  scale_y_continuous(name = expression(bold(atop(atop("","Percent of the Variance Explained by"),atop(s[het]~"Compared to the Null Model:",has.children %~% y.axis.covariates + age + age^2 + PC1..PC30))))) +
  scale_x_discrete(name = "") +
  sex.colours.fill +
  coord_flip() + 
...
get.sig.code <- function(should.check,mod,covar,s) {
  if (should.check == 0) {
    ...
    curr.model <- figure.4.subset.plot[curr.cov.string == mod & sex == s & variant.type == "DEL",model][[1]]
    if (curr.model[term == to.check,p.value] < (0.001)) {
      return("***")
    } else if (curr.model[term == to.check,p.value] < (0.01)) {
      return("**")
...
```{r Main Text Figure 4, fig.height=5.5, fig.width=13}

figure.4 <- labels.plot + ors.plot + inc.r.shet.plot + fitness.redux + plot_layout(ncol = 4, guides = "collect", widths = c(0.4,1.3,0.3,0.7))
figure.4

ggsave("figures/Figure4.svg",figure.4,height = 5.5, width = 13)
...
birth.stats <- UKBB.phenotype.data[,c("eid","children.fathered","live.births","birthday","agePulse","sexPulse")]
birth.stats[,dummy:=1]
...
plot.childlessness <- ggplot(agePlot,aes(agePulse,mean.childlessness*100,group=y.var,colour=sexPulse)) + geom_point(position=position_dodge(0.5)) + geom_errorbar(aes(ymin=(mean.childlessness-ci.childlessness)*100,ymax=(mean.childlessness+ci.childlessness)*100),width=0,position=position_dodge(0.5)) + xlab("Age") + ylab("% Childless") + sex.colours.colour + theme.figures.legend

## Plot of birth year cohorts
birth.stats[,year:=format(birthday,"%Y")]
yearPlot <- birth.stats[,sum(dummy),by=c("year","sexPulse")]
yearPlot[,year:=as.integer(year)]
...
plot.birthyears <- ggplot(yearPlot,aes(year,prop,group=sexPulse,fill=sexPulse)) + 
...
ggsave("figures/supplement/SuppFig4.png",linear.data[[2]], dpi = 300, height = 3, width = 8.5, units = "in")
...
```{r Supp Fig 5, fig.height=4, fig.width=8.5}
...
ggsave("figures/supplement/SuppFig5.png",plot.all.variants, dpi = 300, height = 4, width = 8.5, units = "in")
...
ggsave("figures/supplement/SuppFig6.png",remove.zero[[2]], dpi = 300, height = 3, width = 8.5, units = "in")
```
...
```{r Supp Fig 8, fig.height=3, fig.width=8.5}

high.maf.data <- make.meta.table(results.fertility, F, allele.freq = 1e-3, b=0.25)
high.maf.data

ggsave("figures/supplement/SuppFig8.png",high.maf.data[[2]], dpi = 300, height = 3, width = 8.5, units = "in")

make.meta.table(results.fertility.no.mhq, F, allele.freq = 0, b=0.25)
plot.a
```
...
```{r Supp Fig 10, fig.height=5, fig.width=10}
...
ggsave("figures/supplement/SuppFig10.png",plot.testis, dpi = 300, height = 5, width = 10, units = "in")
...
male.icd <- ggplot(hes.plot.table[sex == "MALE"],aes(var.p.log,factor*icd.p.log,colour=chapter)) + 
  geom_point(size=0.5) +
  scale_x_continuous(name = "", breaks = c(11.9,12.4,12.9,13.4), limits = c(11.8,13.6)) +
  ...
  geom_text(data = hes.plot.table[sex == "MALE" & (var.p.log < 12.5 | icd.p.log > 22)],aes(label=meaning),size=3) +
  ggtitle("Males") +
...
female.icd <- ggplot(hes.plot.table[sex == "FEMALE"],aes(var.p.log,factor*icd.p.log,colour=chapter)) + 
  geom_point(size=0.5) + 
  scale_x_continuous(name = "", breaks = c(1.2,1.3,1.4,1.5), limits = c(1.15,1.55)) +
  scale_y_continuous(name = "", limits = c(-100,100)) +
  geom_text(data = hes.plot.table[sex == "FEMALE" & (var.p.log < 1.35 | icd.p.log > 22) & chapter != "XV"],aes(label=meaning),size=3) +
  ggtitle("Females") +
...
male.fi <- ggplot(fi.plot.table[sex == "MALE"],aes(var.p.log,factor*icd.p.log,colour=chapter)) + 
  ...
  geom_text(data = fi.plot.table[sex == "MALE" & (var.p.log < 12.8 | icd.p.log > 22)],aes(label=meaning),size=3,nudge_y=0.15) +
  theme.figures + theme(plot.title = element_text(hjust = 0.5))
...
female.fi <- ggplot(fi.plot.table[sex == "FEMALE"],aes(var.p.log,factor*icd.p.log,colour=chapter)) + 
  ...
  scale_x_continuous(name = "") +
  scale_y_continuous(name = "", limits = c(-100,100)) +
  geom_text(data = fi.plot.table[sex == "FEMALE" & (var.p.log < 1.35 | icd.p.log > 22) & chapter != "XV"],aes(label=meaning),size=3,nudge_y=0.15) +
...
combined.fi <- combined.fi.top / combined.fi.bottom + plot_layout(heights=c(10,0.05)) + plot_annotation(tag_levels = "A")
combined.fi
...
```{r Supp Fig 12/13, fig.height=9, fig.width=8}

male.plots.1 <- plot.medical("MALE",1,F,"Chapters - Complete Health Outcomes Data", "Hospital Episode Stats.")
male.plots.2 <- plot.medical("MALE",2,F,"Disease Groups")
male.plots.3 <- plot.medical("MALE",3,T,"Specific Diseases")

y.lab <- grid::textGrob(expression(bold(-log[10]~p~value~`for`~the~effect~of~s[het]~burden==1~on~having~children)), rot=90)

meta.icd.plot.male <- male.plots.1[[1]] + male.plots.1[[2]] + male.plots.2[[1]] + male.plots.2[[2]] + male.plots.3[[1]] + male.plots.3[[2]] + plot_layout(ncol = 2, nrow = 3, widths = c(16, 6), tag_level = 'new') + plot_annotation(tag_levels = "A")

meta.icd.plot.male <- wrap_elements(y.lab) + meta.icd.plot.male + plot_layout(ncol = 2, nrow = 1, widths = c(0.3,20), tag_level = 'keep')
meta.icd.plot.male

ggsave("figures/supplement/SuppFig12.svg",meta.icd.plot.male,width=8,height=9)

female.plots.1 <- plot.medical("FEMALE",1,F,"Chapters - Complete Health Outcomes Data", "Hospital Episode Stats.")
female.plots.2 <- plot.medical("FEMALE",2,F,"Disease Groups")
female.plots.3 <- plot.medical("FEMALE",3,T,"Specific Diseases")

meta.icd.plot.female <- female.plots.1[[1]] + female.plots.1[[2]] + female.plots.2[[1]] + female.plots.2[[2]] + female.plots.3[[1]] + female.plots.3[[2]] + plot_layout(ncol = 2, nrow = 3, widths = c(16, 6), tag_level = 'new') + plot_annotation(tag_levels = "A")
...
ggsave("figures/supplement/SuppFig13.svg",meta.icd.plot.female,width=8,height=9)

```

#### Figure 14.

```{r Supp Fig 14, fig.height=8, fig.width=8}
...
## Have to do had.sex seperately because the effect is so strong
data <- data.table(sexPulse=c(1,2))
...
ggsave("figures/supplement/SuppFig14.png",plot.betas, dpi = 300, height = 8, width = 8, units = "in")
...
```{r Supp Fig 15, fig.height=3, fig.width=8.5}

same.sex <- make.meta.table(results.same.sex, F, b=0.5, ymin=-0.8, ymax = 4.5, p.pos = -0.2, block = -0.5)
same.sex

ggsave("figures/supplement/SuppFig15.png",same.sex[[2]], dpi = 600, height = 3, width = 8.5, units = "in")
...
```{r Supp Fig 16, fig.height = 3, fig.width = 8.5}
...
ggsave("figures/supplement/SuppFig16.png",cognition.fertility.plot,width=8.5,height=3,units = "in", dpi = 300)
```

#### Figure 17.

```{r Supp Fig 17, fig.height=4, fig.width=8}
...
ggsave("figures/supplement/SuppFig17.png",cog.fit.plots, dpi = 300, height = 5, width = 8, units = "in")
```

#### Figure 18.

```{r Supp Fig 18, fig.height=9, fig.width=8.5}
...
ggsave("figures/supplement/SuppFig18.png",bias.plots, dpi = 300, height = 9, width = 8.5, units = "in")
...
answered.mhq.data[[1]][ variant.type == "META",paste0(Sex, " Answered MHQ OR=",sprintf("%0.2f",var.beta), " [95% CI ",sprintf("%0.2f",exp(log(var.beta)-(1.96*var.stderr))), "-",sprintf("%0.2f",exp(log(var.beta)+(1.96*var.stderr))),"]", " p=",if_else(var.p<=1e-2,sprintf("%0.1e",var.p),sprintf("%0.2f",var.p)))]
...
```{r Supp Figure 19, fig.height=4, fig.width=8.5}
...
plot.separate <- ggplot(res[source == "fi" & variant.type == "META"],aes(trait,var.beta,group=Sex,colour=Sex)) +
  ...
  geom_errorbar(aes(ymin=var.ci.lower,ymax=var.ci.upper),width=0,position=position_dodge(0.5)) +
  geom_text(aes(y = 9.0e-5, label=paste0("p = ", sprintf("%0.2g",var.p))),position=position_dodge(0.5),size=4,hjust=1,show.legend = F) +
  ...
  scale_y_log10(name=expression(bold(Odds~Ratio~at~s[het]~burden==1~of~having~given~disorder)),limits = c(1e-5,10000),breaks = c(1/(10^c(4:1)),10^c(0:4)),labels = c("0.0001","0.001","0.01","0.1","1","10","100","1000","10000")) +
  scale_shape_identity() +
  scale_size_area(breaks=c(50000,100000,150000),guide=guide_legend(title="# of Indivs.")) +
  sex.colours.colour.rev +
  coord_flip() +
...
ggsave("figures/supplement/SuppFig19.svg", plot.separate, width = 8.5, height = 4, dpi = 300)

```

#### Figure 20.

```{r Supp Figure 20, fig.height=11, fig.width=10}
...
ors.plot <- ggplot(final.results.matrix[variant.type == "META"], aes(curr.cov.string, or, group = sexPulse, colour = sexPulse)) +
  ...
  geom_text(aes(y = -0.13, label = if_else(var.p<=1e-2,sprintf("%0.1e",var.p),sprintf("%0.2f",var.p))), position = position_dodge(0.90), hjust = 0,size = 4) +
  scale_x_discrete(name = "") +
  scale_y_continuous(name = expression(bold(atop(atop(Odds~Ratio~at~s[het]~burden==1,on~having~children),atop("","")))),limits = c(-0.15, 1.21), breaks = seq(0.2,1.2,by=0.2), expand = c(0,0)) +
  scale_size_area(breaks=c(150000,175000,180000),guide=guide_legend(title="# of Indivs."), limits = c(0, 200000),) +
  sex.colours.colour +
...
r2.plot.table <- final.results.matrix[grepl("Fluid Intelligence", curr.cov.string) == F & variant.type == "DEL"]
r2.plot.table[,val:=if_else(sexPulse == "Male",
                            inc.r.shet/r2.plot.table[sex == 1 & curr.cov == "",inc.r.shet],
                            inc.r.shet/r2.plot.table[sex == 2 & curr.cov == "",inc.r.shet])]
r2.plot.table[,val:=(val)*100]
...
inc.r.shet.plot <- ggplot(r2.plot.table,aes(curr.cov.string, val, group = sexPulse, fill = sexPulse)) +
  geom_hline(yintercept=100,colour="red",linetype = 2, size = 1) +
  geom_col(position = position_dodge(0.75), size = 1, width = 0.75, colour = "black") +
  scale_y_continuous(name = expression(bold(atop(atop("","Proportion of Variance Explained by"),atop(s[het]~"Compared to the Null Model:",has.children %~% y.axis.covars + age + age^2 + PC1..PC30))))) +
  # scale_y_continuous(name = expression(bold(atop(Nagelkerke~Incremental~r^2,Null~":"~has.children %~% y.axis.covars + age + age^2 + PC1..PC30)))) +
  scale_x_discrete(name = "") +
  sex.colours.fill +
  coord_flip() + 
...
get.sig.code <- function(should.check,mod,covar,s) {
  if (should.check == 0) {
    ...
    curr.model <- final.results.matrix[curr.cov.string == mod & sex == s & variant.type == "DEL",model][[1]]
    if (curr.model[term == to.check,p.value] < (0.001)) {
      return("***")
    } else if (curr.model[term == to.check,p.value] < (0.01)) {
      return("**")
...
joint.plot
ggsave("figures/supplement/SuppFig20.png", joint.plot, width = 10, height = 11, dpi = 300)

```

#### Figure 21.

```{r Supp Fig 21, fig.height=3, fig.width=4}
...
ggsave("figures/supplement/SuppFig21.png",plot.child.fert,width=4,height=3,units = "in", dpi = 300)

```

#### Figure 22.

```{r Supp Fig 22, fig.height=7.5, fig.width=8.5}
...
ggsave("figures/supplement/SuppFig22.png",shet.comp.plot, dpi = 300, height = 7.5, width = 8.5, units = "in")
...
```{r Figure 23, fig.height=7, fig.width=8.5}
...
ggsave("figures/supplement/SuppFig23.png",plot.neutrals, dpi = 600, height = 7, width = 8.5, units = "in")
```

#### Figure 24.

```{r Figure 24, fig.height=5, fig.width=7}
...
ggsave("figures/supplement/SuppFig24.png",pc.plots, dpi = 300, height = 5, width = 7, units = "in")

```

#### Figure 25.

```{r Supp Fig 25, fig.height = 6, fig.width = 8.5}
...
ggsave("figures/supplement/SuppFig25.png",mh.fertility.plot, dpi = 300, height = 6, width = 8.5, units = "in")
...
# shet on phenotype
## Figure 1
supptable4a <- format.table(plot.a[[1]],"Childlessness",T,"Figure1A") ## Main fertility
...
supptable4a <- bind_rows(supptable4a,format.table(fig.hhi[[1]],"Household Income",F,"Figure3D")) ## HHI
supptable4a <- bind_rows(supptable4a,format.table(fig.cog[[1]],"Fluid Intel.",F,"Figure3E")) ## Cognition
supptable4a <- bind_rows(supptable4a,format.table(fig.mht[[1]],"Has MHT",F,"Figure3F")) ## Cognition

## Sup Fig 3
supptable4a <- bind_rows(supptable4a,format.table(linear.data[[1]],"Num. Children",F,"SupFig3")) ## Linear rather than logistic fertility
...
## Supp Fig 6
supptable4a <- bind_rows(supptable4a,format.table(gene.data.pli[[1]],"high pLI",T,"SupFig6A")) ## pli on childlessness
supptable4a <- bind_rows(supptable4a,format.table(gene.data.shet[[1]],"high sHET",T,"SupFig6B")) ## shet on childlessness

## Sup Fig 7
supptable4a <- bind_rows(supptable4a,format.table(high.maf.data[[1]],"MAF ≤1e-3",T,"SupFig7")) ## maf ≤1e-3 variants

## Sup Fig 8
supptable4a <- bind_rows(supptable4a,format.table(res.age[age==40],"Age Group 40-50",T,"SupFig8")) ## age 40-50
supptable4a <- bind_rows(supptable4a,format.table(res.age[age==50],"Age Group 50-60",T,"SupFig8")) ## age 50-60
supptable4a <- bind_rows(supptable4a,format.table(res.age[age==60],"Age Group 60-70",T,"SupFig8")) ## age 60-70

## Sup Fig 14
supptable4a <- bind_rows(supptable4a,format.table(same.sex[[1]],"Same Sex Sexual Behaviour",T,"SupFig14")) ## Same sex sexual behaviour

## Sup Fig 17
supptable4a <- bind_rows(supptable4a,format.table(email.data[[1]],"Has Email",T,"SupFig17A")) ## email
supptable4a <- bind_rows(supptable4a,format.table(answered.mhq.data[[1]],"Answered MHQ",T,"SupFig17B")) ## answered MHQ
supptable4a <- bind_rows(supptable4a,format.table(has.gp.data[[1]],"Has GP Data",T,"SupFig17C")) ## answered MHQ

## Sup Fig 18
supptable4a <- bind_rows(supptable4a,format.table(res[trait == "developmental_disorder" & source == "fi"],"Has DD/ID",T,"SupFig18")) ## DD/ID
supptable4a <- bind_rows(supptable4a,format.table(res[trait == "asd" & source == "fi"],"Has ASD",T,"SupFig18")) ## ASD
supptable4a <- bind_rows(supptable4a,format.table(res[trait == "add" & source == "fi"],"Has ADHD",T,"SupFig18")) ## ADHD
supptable4a <- bind_rows(supptable4a,format.table(res[trait == "scizo" & source == "fi"],"Has Schizophrenia",T,"SupFig18")) ## Schizo.
supptable4a <- bind_rows(supptable4a,format.table(res[trait == "bipolar" & source == "fi"],"Has Bipolar Disorder",T,"SupFig18")) ## Bipolar Dis.

## Sup Fig 22
supptable4a <- bind_rows(supptable4a,format.table(cassa.data[[1]],"Cassa sHET",T,"SupFig22C")) ## answered MHQ

## Sup Fig 23
supptable4a <- bind_rows(supptable4a,format.table(plot.fruit[[1]],"Fresh Fruit Intake",T,"SupFig23A")) ## answered MHQ
supptable4a <- bind_rows(supptable4a,format.table(plot.hands[[1]],"Left Handed",T,"SupFig23B")) ## answered MHQ
supptable4a <- bind_rows(supptable4a,format.table(plot.hair[[1]],"Blonde Hair Colour",T,"SupFig23C")) ## answered MHQ
...
supptable4b <- format.table(partner.beta.plot[[1]], "Partner  At Home","SupFig13A")
supptable4b <- bind_rows(supptable4b,format.table(had.sex.beta.plot[[1]], "Ever Had Sex","SupFig13B"))
supptable4b <- bind_rows(supptable4b,format.table(ea.beta.plot[[1]], "Educational Attainment","SupFig13C"))
supptable4b <- bind_rows(supptable4b,format.table(hhi.beta.plot[[1]], "Household Income","SupFig13D"))
supptable4b <- bind_rows(supptable4b,format.table(cog.beta.plot[[1]], "Fluid Intel.","SupFig13E"))
supptable4b <- bind_rows(supptable4b,format.table(mhq.beta.plot[[1]], "Mental Health Traits","SupFig13F"))
supptable4b <- bind_rows(supptable4b,format.table(same.sex.beta.plot[[1]], "Same Sex Sexual Behaviour","SupFig13G"))
...
supptable4a[,formated:=if_else(`Effect Type`=="OR",
...
supptable4b[,formated:=paste0("OR=",sprintf("%0.2f",`Effect (OR/Beta)`), " [95% CI ",sprintf("%0.2f",exp(log(`Effect (OR/Beta)`)-(1.96*`std. err.`))), "-",sprintf("%0.2f",exp(log(`Effect (OR/Beta)`)+(1.96*`std. err.`))),"]", " p=",if_else(`p. value`<=1e-2,sprintf("%0.1e",`p. value`),sprintf("%0.2f",`p. value`)))]
```
...
plot.a[[1]][ variant.type == "META",paste0("Primary Result ", Sex, " OR=",sprintf("%0.2f",var.beta), " [95% CI ",sprintf("%0.2f",exp(log(var.beta)-(1.96*var.stderr))), "-",sprintf("%0.2f",exp(log(var.beta)+(1.96*var.stderr))),"]", " p=",if_else(var.p<=1e-2,sprintf("%0.1e",var.p),sprintf("%0.2f",var.p)))]
paste0("")
...
# P. value reduction
paste0("P. value for base model    : ", sprintf("%0.1e",plot.a[[1]][variant.type == "META" & Sex == "Male", var.p]))
paste0("P. value for model w/DD:ID : ", sprintf("%0.1e",fi.analysis.table[variant.type == "META" & coding == "F81" & sex == "MALE", var.p]))
paste0("")
...
fig.had.sex[[1]][variant.type == "META",paste0(Sex, " Fertility Gene OR=",sprintf("%0.2f",var.beta), " [95% CI ",sprintf("%0.2f",exp(log(var.beta)-(1.96*var.stderr))), "-",sprintf("%0.2f",exp(log(var.beta)+(1.96*var.stderr))),"]", " p=",if_else(var.p<=1e-2,sprintf("%0.1e",var.p),sprintf("%0.2f",var.p)))]
...
paste0(r2.plot.table[curr.cov.string == "Has Partner, Ever Had Sex" & sexPulse == "Male", paste0("Contribution of Partner & Had Sex to Fitness in ", sexPulse, "s : ",sprintf("%0.0f%%",100-val))])
paste0("")
...
## Total SNVs:
paste0("Number of SNV/INDel Variants: ", nrow(variants.UKBB[,]))
paste0("Number of individuals with WES data: ", nrow(UKBB.phenotype.data[has.wes==T]))
paste0("")
